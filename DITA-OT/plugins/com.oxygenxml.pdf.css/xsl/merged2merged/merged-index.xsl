<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="opentopic-index:index.groups//opentopic-index:formatted-value">
    <xsl:copy>
      ...
      <xsl:attribute name="id" select="
          string-join(
          for $indexid in following-sibling::opentopic-index:refID/@indexid
          return generate-id($indexid),
          ' ')"/>
  ...
  <xsl:template match="opentopic-index:index.groups//opentopic-index:refID[@value]">
    <xsl:copy>
      <xsl:apply-templates select="@* except @indexid"/>
      <xsl:attribute name="indexid" select="generate-id(@indexid)"/>
      <xsl:call-template name="generate-index-element-class"/>
