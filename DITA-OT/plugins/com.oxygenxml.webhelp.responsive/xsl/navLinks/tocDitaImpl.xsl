...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:xhtml="http://www.w3.org/1999/xhtml" version="3.0">
    ...
    <xsl:key name="tocHrefs"
        ...
        use="@href"/>
    <xsl:key name="topicrefHrefs"
        match="topicref[@href][not(@href = $VOID_HREF)][not(@processing-role = 'resource-only')]"
        use="@href"/>
    ...
    <!-- 
        WH-3331 - Display xref target navtitle.
    -->
    <xsl:template match="*[contains(@class, ' topic/xref ')][empty(text())]" mode="fix-shortdesc" priority="20">
        <xsl:variable name="nodes" select="key('topicrefHrefs', @href)"/>
        <xsl:choose>
            <xsl:when test="count($nodes) = 1">
                <xsl:apply-templates select="$nodes[1]" mode="get-navtitle"/>
            </xsl:when>
            <xsl:otherwise>
