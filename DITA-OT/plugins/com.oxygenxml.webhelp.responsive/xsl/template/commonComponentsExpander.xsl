...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- 
        Expand references to WebHelp CSS.
        The CSS references are stored in HTML Fragment extension files 
        in the plugin's base directory.
    -->
    <xsl:template match="whc:page_css[@page]" mode="copy_template">
        ...
        <xsl:variable name="pageLibRefsDir" select="concat($whPluginDir, '/oxygen-webhelp/page-templates-fragments/css/')"/>
        <xsl:variable name="pageLibRefsFileName">
            <xsl:choose>
                <xsl:when test="@page = 'main'">
                    <xsl:value-of select="'main-page-css.xml'"/>
                </xsl:when>
                <xsl:when test="@page = 'topic'">
                    <xsl:value-of select="'topic-page-css.xml'"/>
                </xsl:when>
                <xsl:when test="@page = 'search'">
                    <xsl:value-of select="'search-page-css.xml'"/>
                </xsl:when>
                <xsl:when test="@page = 'indexterms'">
                    <xsl:value-of select="'indexterms-page-css.xml'"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <!-- 
        ...
        The JS references are stored in HTML Fragment extension files 
        in the plugin's base directory.
    -->
            ...
            <xsl:when test="string-length($cssResources) > 0">
                ...
                <xsl:for-each select="$cssResList">
                    <link rel="stylesheet" type="text/css" href="{
                        ...
                        .,
                        '?buildId=', 
                        $WEBHELP_TRANSFORMATION_TIMESTAMP)
                        }"/>                    
                </xsl:for-each>                
            </xsl:when>
            <xsl:otherwise>
                    ...
                    <xsl:choose>
                       <xsl:when test="string-length($WEBHELP_LOGO_IMAGE) > 0 
                        ...
                        <xsl:when
                           ...
                           <img src="{concat($PATH2PROJ, $WEBHELP_LOGO_IMAGE, '?buildId=', $WEBHELP_TRANSFORMATION_TIMESTAMP)}" alt="{$altText}"/>
                        </xsl:when>
                    </xsl:choose>        
                </a>
            </xsl:variable>
            ...
            <a target="_blank" >
                <xsl:attribute name="href" >
                    ...
                    <xsl:variable name="pdfLinkContext">
                        <xsl:if test="(oxy:getParameter('webhelp.pdf.link.named.destination.enabled') = 'yes'
                            or oxy:getParameter('webhelp.pdf.link.name.destination.enabled') = 'yes')
                            and exists($ditaot_topicContent)">
    ...
    <!-- WH-3387: Add attribute on header and topic pages when the 'webhelp.full.page.width' parameter is set to 'yes' -->
    <xsl:template match="*[contains(@class, 'wh_header') or contains(@id, 'wh_topic_container')]//@class[contains(., 'container-fluid')]" mode="copy_template">
        <xsl:copy-of select="." copy-namespaces="no"/>
        <xsl:if test="oxy:getParameter('webhelp.full.page.width') = 'yes'">
            <xsl:attribute name="data-full-page-width" select="'yes'"/>
        </xsl:if>
    </xsl:template>
    ...
    <!-- 
        Expand the 'webhelp_theme_button'.
    -->
    <xsl:template match="whc:webhelp_theme_button" mode="copy_template">
        <!-- EXM-36737 - Context node used for messages localization -->
        <xsl:param name="i18n_context" tunnel="yes" as="element()*"/>
        <xsl:if test="oxy:getParameter('webhelp.enable.dark.mode') = 'yes'">
            <xsl:variable name="switchTheme">
                <xsl:choose>
                    <xsl:when test="exists($i18n_context)">
                        <xsl:for-each select="$i18n_context[1]">
                            <xsl:call-template name="getWebhelpString">
                                <xsl:with-param name="stringName" select="'switchTheme'"/>
                            </xsl:call-template>
                        </xsl:for-each>
                    </xsl:when>
                    <xsl:otherwise>Switch theme</xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            ...
            <xsl:variable name="theme">
                <div>
                    <xsl:call-template name="generateComponentClassAttribute">
                        <xsl:with-param name="compClass">wh_theme</xsl:with-param>                
                    </xsl:call-template>
                    <xsl:copy-of select="@* except @class"/>                        
                    <button id="wh_theme_button" class="wh_theme_btn" data-wh-theme="auto" title="{$switchTheme}" aria-label="{$switchTheme}" aria-haspopup="menu" aria-expanded="false"/>            
                </div>        
            </xsl:variable>
            
            <xsl:call-template name="outputComponentContent">
                <xsl:with-param name="compContent" select="$theme"/>
                <xsl:with-param name="compName" select="local-name()"/>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
  ...
  -->
    ...
    <xsl:template name="generateFaviconLink">
        <xsl:if test="string-length($WEBHELP_FAVICON) > 0">            
            <xsl:variable name="favIconPath" select="concat($PATH2PROJ, $WEBHELP_FAVICON, '?buildId=', $WEBHELP_TRANSFORMATION_TIMESTAMP)"/>
