...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:variable name="newClassValue">
            <xsl:choose>
                <xsl:when test="exists($publicationToc) and exists($topicToc)">
                    <xsl:value-of select="
                        if (oxy:getParameter('webhelp.full.page.width') = 'yes') then
                            replace(@class, 'col-xxl-7', 'col-xxl-8')
                        else
                            @class"/>
                </xsl:when>
                <xsl:otherwise>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="exists($publicationToc)">
                            ...
                            <xsl:value-of select="
                                if (oxy:getParameter('webhelp.full.page.width') = 'yes') then
                                    concat('col-xxl-10 col-lg-9 col-md-9 col-sm-12', $nonBootstrapClassValues)
                                else
                                    concat('col-xxl-9 col-lg-9 col-md-9 col-sm-12', $nonBootstrapClassValues)"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <!-- Only Topic TOC exists -->
                            <xsl:value-of select="concat('col-xxl-10 col-lg-10 col-md-10 col-sm-10 col-xs-12', $nonBootstrapClassValues)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:template match="nav[@id='wh_publication_toc'][contains(@class, 'col-')]" mode="fix-content-width">
        <xsl:variable name="nonBootstrapClassValues">
            <xsl:for-each select="tokenize(@class, ' ')">
                <xsl:if test="not(starts-with(.,'col-'))">
                    <xsl:value-of select="concat(' ', .)"/>
        ...
        <xsl:variable name="newClassValue" select="
            if (oxy:getParameter('webhelp.full.page.width') = 'yes') then
                concat('col-xxl-2 col-lg-3 col-md-3 col-sm-12', $nonBootstrapClassValues)
            else
                concat('col-xxl-3 col-lg-3 col-md-3 col-sm-12', $nonBootstrapClassValues)"/>
    ...
    <xsl:template match="*[contains(@class, 'topic/link')]" mode="copy-child-links">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <!-- Add description in title attribute. -->
    ...
    <xsl:template match="whc:webhelp_print_link" mode="copy_template">
		...
		<!-- WH-3387 - Full Page Width -->
    <xsl:template match="whc:webhelp_full_width_link" mode="copy_template">
        <xsl:param name="i18n_context" tunnel="yes" as="element()*"/>
        ...
        <xsl:variable name="title">
            <xsl:choose>
                <xsl:when test="exists($i18n_context)">
                    <xsl:for-each select="$i18n_context[1]">
                        <xsl:call-template name="getWebhelpString">
                            <xsl:with-param name="stringName" select="$key"/>
                        </xsl:call-template>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="$key = 'exitFullscreen'">Exit Fullscreen</xsl:when>
                        <xsl:otherwise>Enter Fullscreen</xsl:otherwise>
                    </xsl:choose>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <button class="webhelp_full_width_link" data-next-state="{$state}" aria-label="{$title}" title="{$title}"></button>
    </xsl:template>
    ...
    <!-- WH-3473 - Markdown Menu -->
    <xsl:template match="whc:webhelp_md_menu" mode="copy_template">
        <xsl:param name="i18n_context" tunnel="yes" as="element()*"/>
        <xsl:if test="oxy:getParameter('webhelp.enable.ai.friendly') = 'yes'">
            <xsl:variable name="title">
                <xsl:choose>
                    <xsl:when test="exists($i18n_context)">
                        <xsl:for-each select="$i18n_context[1]">
                            <xsl:call-template name="getWebhelpString">
                                <xsl:with-param name="stringName" select="'mdExportMenu'"/>
                            </xsl:call-template>
                        </xsl:for-each>
                    </xsl:when>
                    <xsl:otherwise>Markdown Export menu</xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <div class="wh-md-menu">
                <button class="wh-md-menu-button" aria-haspopup="menu" aria-expanded="false" aria-label="{$title}" title="{$title}"/>
            </div>
        </xsl:if>
    </xsl:template>
